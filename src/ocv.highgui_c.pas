...
uses
  ocv.core.types_c, ocv.core_c, ocv.lib;
...
type
  TcvCaptureFromAVI = function(const filename: pCvChar): pCvCapture; cdecl;
  TcvCaptureFromCAM = function(index: Longint): pCvCapture; cdecl;
  TcvCaptureFromFile = function(const filename: pCvChar): pCvCapture; cdecl;
  TcvCreateAVIWriter = function(const filename: pCvChar; fourcc: Integer; fps: Double; frame_size: TCvSize; is_color: Integer = 1): pCvVideoWriter; cdecl;
  //TcvCreateButton = function(const button_name: pCvChar = nil; on_change: TCvButtonCallback = nil; userdata: Pointer = nil; button_type: Integer = CV_PUSH_BUTTON; initial_button_state: Integer = 0): Integer; cdecl;
  ...
  TcvCreateTrackbar = function(const trackbar_name: pCvChar; const window_name: pCvChar; value: PInteger; count: Integer; on_change: TCvTrackbarCallback): Integer; cdecl;
  TcvCreateTrackbar2 = function(const trackbar_name: pCvChar; const window_name: pCvChar; value: PInteger; count: Integer; on_change: TCvTrackbarCallback2; userdata: Pointer = nil): Integer; cdecl;
  ...
  TcvFontQt = function(const nameFont: pCvChar; pointSize: Integer ; color: TCvScalar ; weight: Integer = CV_FONT_NORMAL; style: Integer = CV_STYLE_NORMAL; spacing: Integer = 0): TCvFont; cdecl;
  ...
  TcvvCreateTrackbar = function(const trackbar_name: pCvChar; const window_name: pCvChar; value: PInteger; count: Integer; on_change: TCvTrackbarCallback): Integer; cdecl;
  TcvvInitSystem = function(argc: Integer; argv: ppCVChar): Integer; cdecl;
  TcvvLoadImage = function(const filename: pCvChar; iscolor: Integer = CV_LOAD_IMAGE_UNCHANGED): pIplImage; cdecl;
  TcvvNamedWindow = function(const name: pCvChar; flags: Integer = CV_WINDOW_AUTOSIZE): Integer; cdecl;
  TcvvSaveImage = function(const filename: pCvChar; const image: Pointer; const params: PInteger = nil): Integer; cdecl;
  //TcvvWaitKey = function(delay: Integer = 0): Integer; cdecl;
  ...
  TcvWriteToAVI = function(writer: pCvVideoWriter; image: pIplImage): Integer; cdecl;
  ...
  TcvSetPostprocessFuncWin32 = procedure(const callback: Pointer); cdecl;
  TcvSetPreprocessFuncWin32 = procedure(const callback: Pointer); cdecl;
  ...
  TcvShowImage = procedure(const name: pCVChar; const image: pIplImage); cdecl;
  //TcvStopLoop = procedure; cdecl;
  TcvUpdateWindow = procedure(window_name: pCvChar); cdecl;
  TcvvConvertImage = procedure(const src: pCvArr; dst: pCvArr; flags: Integer = 0); cdecl;
  TcvvDestroyWindow = procedure(const name: pCvChar); cdecl;
  TcvvResizeWindow = procedure(name: pCvChar; width: Integer; height: Integer); cdecl;
  TcvvShowImage = procedure(const name: pCvChar; const image: pCvArr); cdecl;
  Tset_postprocess_func = procedure(const callback: Pointer); cdecl;
  Tset_preprocess_func = procedure(const callback: Pointer); cdecl;

var
  cvCaptureFromAVI: TcvCaptureFromAVI;
  cvCaptureFromCAM: TcvCaptureFromCAM;
  cvCaptureFromFile: TcvCaptureFromFile;
  cvCreateAVIWriter: TcvCreateAVIWriter;
  ...
  cvvCreateTrackbar: TcvvCreateTrackbar;
  cvvInitSystem: TcvvInitSystem;
  cvvLoadImage: TcvvLoadImage;
  cvvNamedWindow: TcvvNamedWindow;
  cvvSaveImage: TcvvSaveImage;
  //cvvWaitKey: TcvvWaitKey;
  cvWaitKey: TcvWaitKey;
  cvWriteFrame: TcvWriteFrame;
  cvWriteToAVI: TcvWriteToAVI;
  ...
  cvSetPostprocessFuncWin32: TcvSetPostprocessFuncWin32;
  cvSetPreprocessFuncWin32: TcvSetPreprocessFuncWin32;
  ...
  cvvConvertImage: TcvvConvertImage;
  cvvDestroyWindow: TcvvDestroyWindow;
  cvvResizeWindow: TcvvResizeWindow;
  cvvShowImage: TcvvShowImage;
  set_postprocess_func: Tset_postprocess_func;
  set_preprocess_func: Tset_preprocess_func;
...
procedure Initialize(const Handle: {$IFDEF FPC}TLibHandle{$ELSE}THandle{$IFEND}; const RaiseError: Boolean);
begin
  cvCaptureFromAVI := GetProcAddress(Handle, 'cvCreateFileCapture', RaiseError);
  cvCaptureFromCAM := GetProcAddress(Handle, 'cvCreateCameraCapture', RaiseError);
  cvCaptureFromFile := GetProcAddress(Handle, 'cvCreateFileCapture', RaiseError);
  cvCreateAVIWriter := GetProcAddress(Handle, 'cvCreateVideoWriter', RaiseError);
  //cvCreateButton := GetProcAddress(Handle, 'cvCreateButton', RaiseError);
  cvCreateCameraCapture := GetProcAddress(Handle, 'cvCreateCameraCapture', RaiseError);
  cvCreateFileCapture := GetProcAddress(Handle, 'cvCreateFileCapture', RaiseError);
  cvCreateTrackbar := GetProcAddress(Handle, 'cvCreateTrackbar', RaiseError);
  cvCreateTrackbar2 := GetProcAddress(Handle, 'cvCreateTrackbar2', RaiseError);
  cvCreateVideoWriter := GetProcAddress(Handle, 'cvCreateVideoWriter', RaiseError);
  cvDecodeImage := GetProcAddress(Handle, 'cvDecodeImage', RaiseError);
  cvDecodeImageM := GetProcAddress(Handle, 'cvDecodeImageM', RaiseError);
  cvEncodeImage := GetProcAddress(Handle, 'cvEncodeImage', RaiseError);
  cvFontQt := GetProcAddress(Handle, 'cvFontQt', RaiseError);
  cvGetCaptureDomain := GetProcAddress(Handle, 'cvGetCaptureDomain', RaiseError);
  cvGetCaptureProperty := GetProcAddress(Handle, 'cvGetCaptureProperty', RaiseError);
  cvGetTrackbarPos := GetProcAddress(Handle, 'cvGetTrackbarPos', RaiseError);
  cvGetWindowHandle := GetProcAddress(Handle, 'cvGetWindowHandle', RaiseError);
  cvGetWindowName := GetProcAddress(Handle, 'cvGetWindowName', RaiseError);
  cvGetWindowProperty := GetProcAddress(Handle, 'cvGetWindowProperty', RaiseError);
  cvGrabFrame := GetProcAddress(Handle, 'cvGrabFrame', RaiseError);
  cvInitSystem := GetProcAddress(Handle, 'cvInitSystem', RaiseError);
  cvLoadImage := GetProcAddress(Handle, 'cvLoadImage', RaiseError);
  cvLoadImageM := GetProcAddress(Handle, 'cvLoadImageM', RaiseError);
  cvNamedWindow := GetProcAddress(Handle, 'cvNamedWindow', RaiseError);
  cvQueryFrame := GetProcAddress(Handle, 'cvQueryFrame', RaiseError);
  cvRetrieveFrame := GetProcAddress(Handle, 'cvRetrieveFrame', RaiseError);
  cvSaveImage := GetProcAddress(Handle, 'cvSaveImage', RaiseError);
  cvSetCaptureProperty := GetProcAddress(Handle, 'cvSetCaptureProperty', RaiseError);
  //cvStartLoop := GetProcAddress(Handle, 'cvStartLoop', RaiseError);
  cvStartWindowThread := GetProcAddress(Handle, 'cvStartWindowThread', RaiseError);
  cvvCreateTrackbar := GetProcAddress(Handle, 'cvCreateTrackbar', RaiseError);
  cvvInitSystem := GetProcAddress(Handle, 'cvInitSystem', RaiseError);
  cvvLoadImage := GetProcAddress(Handle, 'cvLoadImage', RaiseError);
  cvvNamedWindow := GetProcAddress(Handle, 'cvNamedWindow', RaiseError);
  cvvSaveImage := GetProcAddress(Handle, 'cvSaveImage', RaiseError);
  //cvvWaitKey := GetProcAddress(Handle, 'cvvWaitKey', RaiseError);
  cvWaitKey := GetProcAddress(Handle, 'cvWaitKey', RaiseError);
  cvWriteFrame := GetProcAddress(Handle, 'cvWriteFrame', RaiseError);
  cvWriteToAVI := GetProcAddress(Handle, 'cvWriteFrame', RaiseError);
  cvAddText := GetProcAddress(Handle, 'cvAddText', RaiseError);
  cvConvertImage := GetProcAddress(Handle, 'cvConvertImage', RaiseError);
  cvDestroyAllWindows := GetProcAddress(Handle, 'cvDestroyAllWindows', RaiseError);
  cvDestroyWindow := GetProcAddress(Handle, 'cvDestroyWindow', RaiseError);
  cvDisplayOverlay := GetProcAddress(Handle, 'cvDisplayOverlay', RaiseError);
  cvDisplayStatusBar := GetProcAddress(Handle, 'cvDisplayStatusBar', RaiseError);
  cvLoadWindowParameters := GetProcAddress(Handle, 'cvLoadWindowParameters', RaiseError);
  cvMoveWindow := GetProcAddress(Handle, 'cvMoveWindow', RaiseError);
  cvReleaseCapture := GetProcAddress(Handle, 'cvReleaseCapture', RaiseError);
  cvReleaseVideoWriter := GetProcAddress(Handle, 'cvReleaseVideoWriter', RaiseError);
  cvResizeWindow := GetProcAddress(Handle, 'cvResizeWindow', RaiseError);
  cvSaveWindowParameters := GetProcAddress(Handle, 'cvSaveWindowParameters', RaiseError);
  cvSetMouseCallback := GetProcAddress(Handle, 'cvSetMouseCallback', RaiseError);
  cvSetOpenGlContext := GetProcAddress(Handle, 'cvSetOpenGlContext', RaiseError);
  cvSetOpenGlDrawCallback := GetProcAddress(Handle, 'cvSetOpenGlDrawCallback', RaiseError);
  cvSetPostprocessFuncWin32 := GetProcAddress(Handle, 'cvSetPostprocessFuncWin32_', RaiseError);
  cvSetPreprocessFuncWin32 := GetProcAddress(Handle, 'cvSetPreprocessFuncWin32_', RaiseError);
  cvSetTrackbarPos := GetProcAddress(Handle, 'cvSetTrackbarPos', RaiseError);
  cvSetWindowProperty := GetProcAddress(Handle, 'cvSetWindowProperty', RaiseError);
  cvShowImage := GetProcAddress(Handle, 'cvShowImage', RaiseError);
  //cvStopLoop := GetProcAddress(Handle, 'cvStopLoop', RaiseError);
  cvUpdateWindow := GetProcAddress(Handle, 'cvUpdateWindow', RaiseError);
  cvvConvertImage := GetProcAddress(Handle, 'cvConvertImage', RaiseError);
  cvvDestroyWindow := GetProcAddress(Handle, 'cvDestroyWindow', RaiseError);
  cvvResizeWindow := GetProcAddress(Handle, 'cvResizeWindow', RaiseError);
  cvvShowImage := GetProcAddress(Handle, 'cvShowImage', RaiseError);
  set_postprocess_func := GetProcAddress(Handle, 'cvSetPostprocessFuncWin32_', RaiseError);
  set_preprocess_func := GetProcAddress(Handle, 'cvSetPreprocessFuncWin32_', RaiseError);
end;
