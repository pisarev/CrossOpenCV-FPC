...
type
  TcvHaarDetectObjects = function(const image: pCvArr; cascade: pCvHaarClassifierCascade; storage: pCvMemStorage; scale_factor: Double ; min_neighbors: Integer ; flags: Integer ; min_size: TCvSize ; max_size: TCvSize  ): pCvSeq; cdecl;
  TcvLatentSvmDetectObjects = function(image: pIplImage; detector: pCvLatentSvmDetector; storage: pCvMemStorage; overlap_threshold: single = 0.5; numThreads: Integer = -1): pCvSeq; cdecl;
  ...
  TcvSetImagesForHaarClassifierCascade = procedure(cascade: pCvHaarClassifierCascade; const sum: pCvArr; const sqsum: pCvArr; const tilted_sum: pCvArr; scale: Double); cdecl;
...
const
  ObjDetectLibName = objdetect_lib;
var
  ObjDetectLibHandle: {$IFDEF FPC}TLibHandle{$ELSE}THandle{$IFEND};
...
procedure Initialize(const Handle: {$IFDEF FPC}TLibHandle{$ELSE}THandle{$IFEND}; const RaiseError: Boolean);
begin
  cvHaarDetectObjects := GetProcAddress(Handle, 'cvHaarDetectObjects', RaiseError);
  cvLatentSvmDetectObjects := GetProcAddress(Handle, 'cvLatentSvmDetectObjects', RaiseError);
  cvLoadHaarClassifierCascade := GetProcAddress(Handle, 'cvLoadHaarClassifierCascade', RaiseError);
  cvLoadLatentSvmDetector := GetProcAddress(Handle, 'cvLoadLatentSvmDetector', RaiseError);
  cvRunHaarClassifierCascade := GetProcAddress(Handle, 'cvRunHaarClassifierCascade', RaiseError);
  cvReleaseHaarClassifierCascade := GetProcAddress(Handle, 'cvReleaseHaarClassifierCascade', RaiseError);
  cvReleaseLatentSvmDetector := GetProcAddress(Handle, 'cvReleaseLatentSvmDetector', RaiseError);
  cvSetImagesForHaarClassifierCascade := GetProcAddress(Handle, 'cvSetImagesForHaarClassifierCascade', RaiseError);
end;
