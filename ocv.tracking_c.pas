...
uses ocv.core.types_c, ocv.imgproc.types_c, ocv.lib;
...
type
  ...
  TcvKalmanUpdateByMeasurement = function(var kalman: TCvKalman; const measurement: pCvMat): pCvMat; cdecl;
  TcvKalmanUpdateByTime = function(var kalman: TCvKalman; const control: pCvMat = nil): pCvMat; cdecl;
  ...
  TcvCalcAffineFlowPyrLK = procedure(const prev: pCvArr; const curr: pCvArr; prev_pyr: pCvArr; curr_pyr: pCvArr; const prev_features: pCvPoint2D32f; var curr_features: TCvPoint2D32f; var matrices: Single; count: Integer; win_size: TCvSize; level: Integer; status: pCVChar; var track_error: Single; criteria: TCvTermCriteria; flags: Integer); cdecl;
  ...
  TcvCalcOpticalFlowPyrLK = procedure(const prev: pIplImage; const curr: pIplImage; prev_pyr: pIplImage; curr_pyr: pIplImage; const prev_features: pCvPoint2D32f; curr_features: pCvPoint2D32f; count: Integer; win_size: TCvSize; level: Integer; status: pCVChar; track_error: PSingle; criteria: TCvTermCriteria; flags: Integer); cdecl;
...
var
  ...
  cvKalmanUpdateByMeasurement: TcvKalmanUpdateByMeasurement;
  cvKalmanUpdateByTime: TcvKalmanUpdateByTime;
...
const
  TrackingLibName = tracking_lib;
var
  TrackingLibHandle: {$IFDEF FPC}TLibHandle{$ELSE}THandle{$IFEND};
...
procedure Initialize(const Handle: {$IFDEF FPC}TLibHandle{$ELSE}THandle{$IFEND}; const RaiseError: Boolean);
begin
  cvCalcGlobalOrientation := GetProcAddress(Handle, 'cvCalcGlobalOrientation', RaiseError);
  cvCamShift := GetProcAddress(Handle, 'cvCamShift', RaiseError);
  cvCreateKalman := GetProcAddress(Handle, 'cvCreateKalman', RaiseError);
  cvEstimateRigidTransform := GetProcAddress(Handle, 'cvEstimateRigidTransform', RaiseError);
  cvKalmanCorrect := GetProcAddress(Handle, 'cvKalmanCorrect', RaiseError);
  cvKalmanPredict := GetProcAddress(Handle, 'cvKalmanPredict', RaiseError);
  cvKalmanUpdateByMeasurement := GetProcAddress(Handle, 'cvKalmanCorrect', RaiseError);
  cvKalmanUpdateByTime := GetProcAddress(Handle, 'cvKalmanPredict', RaiseError);
  cvMeanShift := GetProcAddress(Handle, 'cvMeanShift', RaiseError);
  cvSegmentMotion := GetProcAddress(Handle, 'cvSegmentMotion', RaiseError);
  cvCalcAffineFlowPyrLK := GetProcAddress(Handle, 'cvCalcAffineFlowPyrLK', RaiseError);
  cvCalcMotionGradient := GetProcAddress(Handle, 'cvCalcMotionGradient', RaiseError);
  cvCalcOpticalFlowFarneback := GetProcAddress(Handle, 'cvCalcOpticalFlowFarneback', RaiseError);
  cvCalcOpticalFlowPyrLK := GetProcAddress(Handle, 'cvCalcOpticalFlowPyrLK', RaiseError);
  cvReleaseKalman := GetProcAddress(Handle, 'cvReleaseKalman', RaiseError);
  cvUpdateMotionHistory := GetProcAddress(Handle, 'cvUpdateMotionHistory', RaiseError);
end;
